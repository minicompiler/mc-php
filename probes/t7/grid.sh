...
#   sh probes/t7/grid.sh <snapshot-binary> <out-dir> [all]
set -eu
...
for d in tests/lang Zend/tests ext/standard/tests/strings; do
    ...
    find "php-src/$d" -name '*.phpt' | python3 probes/t0/phpt-run.py \
        --candidate "$root/probes/t7/mcphp.sh" --jobs "${T7_JOBS:-12}" --quiet --out "$out/$n"
done
if [ -n "$full" ]; then
    printf '== the whole corpus ==\n'
    find php-src -name '*.phpt' -not -path '*/sapi/*' | python3 probes/t0/phpt-run.py \
        --candidate "$root/probes/t7/mcphp.sh" --jobs "${T7_JOBS:-12}" --quiet --out "$out/all"
fi

...
echo ""
echo "== 0. the compiler =="
python3 probes/t8/lencheck.py
python3 probes/t8/aritycheck.py
# mc --exe must not overwrite a signed executable at the same inode: the
# kernel kills the next run with SIGKILL (mc's M12 note).
rm -f probes/t8/mc-php
"$MC" --exe probes/t8/mc-php.mc -o probes/t8/mc-php
...
run_grid() {
    ...
    find "$SRC/$dir" -name '*.phpt' \
        | python3 probes/t0/phpt-run.py --candidate "$root/probes/t8/mcphp.sh" \

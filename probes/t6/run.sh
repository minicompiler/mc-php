#!/bin/sh
# T6 -- how far does the wrong-reason table move? (docs/plan.md section 4 row T6)
#
...
nr=0
nrok=0
for f in probes/t6/r/*.php; do
    nr=$((nr + 1))
    msg=$(probes/t6/mcphp.sh "$f" 2>&1 >/dev/null | sed 's/^[^:]*:[0-9]*: //' | head -1)
    probes/t6/mcphp.sh "$f" >/dev/null 2>&1 && rc=0 || rc=$?
    case "$rc:$msg" in
...
run_grid() {
    ...
    find "$SRC/$dir" -name '*.phpt' \
        | python3 probes/t0/phpt-run.py --candidate "$root/probes/t6/mcphp.sh" \
...
MCPHP_BIN="$root/probes/t6/mc-php" python3 probes/t6/why.py "$OUT/why.tsv" < "$OUT/why.list"
python3 probes/t6/whytable.py "$OUT/why.tsv" | tee "$OUT/why.table"

echo ""
[ "$fail" = 0 ] || { echo "T6: something did not measure"; exit 1; }
echo "T6: measured -- see probes/t6/RESULTS.md"

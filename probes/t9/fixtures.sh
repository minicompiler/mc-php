...
cp probes/t9/mc-php "$snap"
MCPHP_BIN=$snap
export MCPHP_BIN
trap 'rm -f "$snap"' EXIT INT TERM
fail=0
ng=0; nok=0
for f in probes/t9/g/*.php; do
    ng=$((ng + 1))
    exp=$("$PHP" "$f" 2>&1) && pe=0 || pe=$?
    got=$(probes/t9/mcphp.sh "$f" 2>&1) && me=0 || me=$?
...
nr=0; nrok=0
for f in probes/t9/r/*.php; do
    nr=$((nr + 1))
    msg=$(probes/t9/mcphp.sh "$f" 2>&1 >/dev/null | sed 's/^[^:]*:[0-9]*: //' | head -1)
    probes/t9/mcphp.sh "$f" >/dev/null 2>&1 && rc=0 || rc=$?
    case "$rc:$msg" in

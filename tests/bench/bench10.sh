#!/bin/sh
# D8 (b): the workload timed under `php` and as an mc-php binary.
# Carved from probes/t10/bench/bench10.sh; the probe keeps its own copy.
#
...
BIN=${BIN:-$root/build/mc-php}
cd "$root"
MC=${MC:-mc}
PHP=${PHP:-php}
REPS=${REPS:-7}

[ -x "$BIN" ] || { echo "bench: no $BIN -- run \`mc build\`"; exit 1; }
...
stamp=$(date -u +%Y-%m-%d)
rec=tests/bench/results/$stamp.json
...
for prog in main.php heavy.php; do
    ...
    if lim env -u MCPHP_OUT -u MCPHP_BIN -u MCPHP_TMP \
        "$PHP" "tests/bench/$prog" > "$tmp/a.out" 2> "$tmp/a.err"
    then :; else ae=$?; fi
    php_to=$timedout
    be=0
    ...
    LIM_SECS=$LIM_SECS python3 tests/bench/time2.py "$PHP" "$tmp/bench" "$REPS" \
        "tests/bench/$prog" --json "$tmp/t.json"
    [ "$first" = 1 ] || printf ',\n' >> "$part"
    first=0

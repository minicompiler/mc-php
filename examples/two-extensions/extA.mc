...
// compiler is missing to write it from extA.php.
//
// Extension A publishes a plain C-level symbol, mcx_add. Nothing php-specific
// about it: mc emits it as a global, and the link exports every global.
//
// A TEMPLATE: tests/examples.sh fills @API@, @DEBUG@, @ZTS@ and @BUILDID@
// from `php -i` of the php that will load it, as reference/extgen.sh does.

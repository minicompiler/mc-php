...
and on every CI leg, main's run after batch E (the merge of #20) against this pull request's run
(runners differ between runs by up to ~40% in absolute time, so the ratio is what compares; only
`macos-15` has `php-config` and `cc` for the C column):

| host | main: interpreted / module | ratio | decimal-c: interpreted / module | ratio |
|---|---|---|---|---|
| macos/arm64 (`macos-15`) | 2.099 / 1.248 ms, twin 0.172 | 1.68x, module/C 7.3 | 2.072 / 0.790 ms, twin 0.171 | **2.62x**, module/C **4.6** |
| linux/aarch64 (`ubuntu-24.04-arm`, container) | 3.091 / 1.775 ms | 1.74x | 3.089 / 1.167 ms | **2.65x** |
| linux/x86_64 (`ubuntu-24.04`, container) | 2.980 / 2.194 ms | 1.36x | 3.709 / 1.442 ms | **2.57x** |
| windows/aarch64 (`windows-11-arm`, x64 php emulated) | 8.778 / 2.942 ms | 2.98x | 8.814 / 1.864 ms | **4.73x** |
| windows/x86_64 (`windows-latest`) | 6.510 / 2.349 ms | 2.77x | 3.573 / 0.914 ms | **3.91x** |

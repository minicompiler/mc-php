// HAND-WRITTEN mc, not mc-php output. awaitable.src.php is the PHP source
// the compiler is working toward; README.md lists the named refusals that
// stand between the two, and tests/examples.sh pins the first of them.
//
...
// url on its own OS thread and returns when all of them are in, and parallel()
// runs ANY php callable in a forked child per argument.
// Sync: \awaitable\Semaphore (bind() caps concurrency), WaitGroup, Mutex --
// each one mutex + condvar, the same shape on every POSIX host.
//
// A TEMPLATE: tests/examples.sh fills @API@, @DEBUG@, @ZTS@ and @BUILDID@
// from `php -i` of the php that will load it (reference/extgen.sh's road),
// and @RTLD_DEFAULT@ from the host -- dlsym's "search every global image"
// handle is (void *)-2 on macOS and (void *)0 on Linux. POSIX only: pthreads,
// fork, pipe and dlsym have no Windows form here (README.md).
#include <prelude>
...
// curl_easy_setopt and curl_easy_getinfo are C VARIADIC functions, and where
// the variadic argument travels is the ABI's: Apple arm64 puts it on the STACK
// after the eight argument registers, so the call pads six zeros to get there;
// AAPCS64 (Linux) and SysV x86-64 put it in the NEXT register, so it is the
// third argument and nothing pads. @VARDECL@ and @VARPAD@ are that difference,
// filled per host by tests/examples.sh -- and it is exactly what the
// `variadic:` of awaitable.src.php's #[Extern] tells a compiler.
extern i32 curl_easy_setopt(uptr h, i64 o, @VARDECL@uptr v);
extern i32 curl_easy_perform(uptr h);
extern i32 curl_easy_getinfo(uptr h, i64 i, @VARDECL@uptr o);
...
#define RTLD_DEFAULT (@RTLD_DEFAULT@)
...
void http_do(uptr job) {
    ...
    curl_easy_setopt(h, CURLOPT_URL, @VARPAD@ld64(job + JOB_URL));
    curl_easy_setopt(h, CURLOPT_WRITEFUNCTION, @VARPAD@&sink);
    curl_easy_setopt(h, CURLOPT_WRITEDATA, @VARPAD@job);
    curl_easy_setopt(h, CURLOPT_FOLLOWLOCATION, @VARPAD@1);
    curl_easy_setopt(h, CURLOPT_TIMEOUT, @VARPAD@30);
    curl_easy_setopt(h, CURLOPT_NOSIGNAL, @VARPAD@1);
    st64(job + JOB_ERR, curl_easy_perform(h));
    u8 c8[8]; uptr c = &c8; st64(c, 0);
    curl_easy_getinfo(h, CURLINFO_RESPONSE_CODE, @VARPAD@c);
